...
{
    // Écrit en C# 6 en raison des limitations de Unity
        ...
        public Item[] GetItemsOfPreset(BulkSpawnPreset preset, byte flag0 = 0x20)
        {
            ...
            switch (preset)
            {
                ...
                case BulkSpawnPreset.CustomFile:
                    ...
                    break;
                default:
                    toRet.Add(new Item(0x09C4)); // branche d'arbre
                    break;
            ...
            if (mul != 1)
            {
                ...
                foreach (var item in toRet)
                    for (int i = 0; i < mul; ++i)
                        multipliedItemList.Add(item); // sont parfaites, elles doivent être copiées à partir de
