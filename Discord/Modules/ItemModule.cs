...
namespace SysBot.ACNHOrders
{
    ...
    public class ItemModule : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task GetItemInfoAsync([Summary("Item ID (in hex)")] string itemHex)
        {
            ...
            if (itemID == Item.NONE)
            {
                await ReplyAsync("Article non valide demandé.").ConfigureAwait(false);
                return;
            }
            ...
            if (result.Length == 0)
                await ReplyAsync($"Aucune donnée de personnalisation disponible pour l'article demandé ({name}).").ConfigureAwait(false);
            else
        ...
        public async Task StackAsync([Summary("Item ID (in hex)")] string itemHex, [Summary("Count of items in the stack")] int count)
        {
            ...
            var ct = count - 1; // valeur 0 => compte de 1
        ...
        public async Task CustomizeAsync([Summary("Item ID (in hex)")] string itemHex, [Summary("Customization value sum")] int sum)
        {
            ...
            if (sum <= 0)
            {
                await ReplyAsync("Aucune donnée de personnalisation spécifiée.").ConfigureAwait(false);
                return;
            }
            ...
            if (remake < 0)
            {
                await ReplyAsync("Aucune donnée de personnalisation disponible pour l'article demandé.").ConfigureAwait(false);
                return;
            }
            ...
            {
                await ReplyAsync("Les données de personnalisation spécifiées sont invalides.").ConfigureAwait(false);
                return;
            }

            var info = ItemRemakeInfoData.List[remake];
            // déjà vérifié les valeurs hors limites du corps/du tissu ci-dessus
            bool hasBody = body == 0 || body <= info.ReBodyPatternNum;
            bool hasFabric = fabric == 0 || info.GetFabricDescription(fabric) != "Invalide";

            if (!hasBody || !hasFabric)
                await ReplyAsync("La personnalisation demandée pour l'article semble être invalide.").ConfigureAwait(false);
